...
import { BarChartIcon, DoubleArrowRightIcon, ImageIcon, LockClosedIcon, MixerVerticalIcon, MoonIcon, SunIcon, VideoIcon } from "@radix-ui/react-icons"
...
export default function FilterPresetsDialog({ currentHardware, updateHardware, currentEffect, updateEffect, updateGameFilter, sort, setSort }: Props) {
  ...
  const [tmpGameFilter, setTmpGameFilter] = useState<GamesTable[]>([])
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        ...
        <DialogContent>
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-2">
              <H4>Effects</H4>
              <ToggleGroup className="flex flex-row flex-wrap justify-start" type="multiple" onValueChange={(value: string[]) => setTmpEffect(value as Enums<'effect_type'>[])} value={tmpEffect}>
                <ToggleGroupItem value="Twilight" className="flex flex-row items-center justify-center gap-x-2">
                  <MoonIcon width={24} height={24} className="fill-current" /> Twilight
                </ToggleGroupItem>
                <ToggleGroupItem value="Video Capture" className="flex flex-row items-center justify-center gap-x-2">
                  <VideoIcon width={24} height={24} className="fill-current" /> Video Capture
                </ToggleGroupItem>
                <ToggleGroupItem value="Sun" className="flex flex-row items-center justify-center gap-x-2">
                  <SunIcon width={24} height={24} className="fill-current" /> Sun
                </ToggleGroupItem>
                <ToggleGroupItem value="Confetti" className="flex flex-row items-center justify-center gap-x-2">
                  <ConfettiSvg width={24} height={24} className="fill-current" /> Confetti
                </ToggleGroupItem>
                <ToggleGroupItem value="Breathing" className="flex flex-row items-center justify-center gap-x-2">
                  <BreathingSVG width={24} height={24} className="fill-current" /> Breathing
                </ToggleGroupItem>
                <ToggleGroupItem value="Wave" className="flex flex-row items-center justify-center gap-x-2">
                  <BarChartIcon width={24} height={24} className="fill-current" /> Wave
                </ToggleGroupItem>
                <ToggleGroupItem value="Swipe" className="flex flex-row items-center justify-center gap-x-2">
                  <DoubleArrowRightIcon width={24} height={24} className="fill-current" /> Swipe
                </ToggleGroupItem>
                <ToggleGroupItem value="Solid" className="flex flex-row items-center justify-center gap-x-2">
                  <LockClosedIcon width={24} height={24} className="fill-current" /> Solid
                </ToggleGroupItem>
                <ToggleGroupItem value="Screen Mirror" className="flex flex-row items-center justify-center gap-x-2">
                  <ImageIcon width={24} height={24} className="fill-current" /> Screen Mirror
                </ToggleGroupItem>
              </ToggleGroup>
            </div>
    ...
    <Drawer open={open} onOpenChange={setOpen}>
      ...
      <DrawerContent>
        ...
        <div className="space-y-3 px-4">
          <div className="space-y-2">
            <H4>Hardware</H4>
            <HardwareToggleGroup currentHardware={currentHardware} updateHardware={updateHardware} classNames="justify-start !ml-0" />
          </div>
          <div className="space-y-2">
            <H4>Sort By:</H4>
            <SortPresetsDropdownMenu dialog sort={sort} setSort={setSort} classNames="justify-start !ml-0" />
          </div>
          <div className="space-y-2">
            <H4>Effects</H4>
            <ToggleGroup className="flex flex-row flex-wrap justify-start" type="multiple" onValueChange={(value: string[]) => setTmpEffect(value as Enums<'effect_type'>[])} value={tmpEffect}>
